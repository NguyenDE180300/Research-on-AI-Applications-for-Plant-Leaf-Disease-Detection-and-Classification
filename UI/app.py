...
# ====== Load mô tả lớp từ JSON ======
with open("class_description.json", "r") as f:
    ...
    if request.method == "POST":
        ...
        if file:
            ...
            for entry in class_info:
                if entry["class_name"] == class_pred:
                    crop = title_case(entry.get("crop_name", ""))
                    disease = title_case(entry.get("diseased_name", ""))
                    description = entry.get("description", "").capitalize()
                    break
